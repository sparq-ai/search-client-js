...
class SearchClientSpec {
  ...
  @test("Get Facet Query Results")
  async testFetchFacetQueryhResults() {
    let result =await new SearchClient("sampleAppId", "sampleSearchToken")
      .searchFields("id")
      .searchFields("name")
      .fields("id")
      .fields("name", "facetField1")
      ...
      .count(40)
      .typoTolerance(2)
      .facetSearch("name", "fq1", "facetField1", 100, "sampleCollectionId");
    expect(result["query"] !== undefined).equal(true);
    expect(result["results"] !== undefined).equal(true)
  }
